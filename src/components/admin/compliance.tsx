...
import { uploadAndExtractOcr, OcrResponse } from "@/services/ocrService";
...
const frameworkTabs: ("All frameworks" | Framework)[] = ["All frameworks", "SOC 2", "GDPR", "ISO 27001", "PCI DSS"];
const PAGE_SIZE = 5;
...
// ─── StatCard ───
function StatCard({ label, rawValue, icon, progress, cardBg, iconBg, delay = 0 }: {
  label: string; rawValue: number; icon: React.ReactNode; progress?: number; cardBg: string; iconBg: string; delay?: number;
}) {
  ...
  return (
    <div className={`rounded-xl border border-slate-100 ${cardBg} px-3 py-3 sm:px-4 sm:py-3.5 flex items-start justify-between gap-2 transition-all duration-300 ease-out cursor-default hover:shadow-lg hover:-translate-y-0.5 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>
...
function FrameworkCard({ fw, delay = 0 }: { fw: Framework; delay?: number }) {
  ...
  useEffect(() => { const t = setTimeout(() => setVisible(true), delay); return () => clearTimeout(t); }, [delay]);
  return (
    <div className={`rounded-xl border border-slate-200 bg-white p-4 sm:p-5 flex flex-col gap-3 transition-all duration-500 ease-out hover:shadow-md hover:-translate-y-0.5 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-5"}`}>
...
function Dropdown({ value, options, open, onToggle, onSelect }: {
  ...
  onToggle: (e: React.MouseEvent) => void; onSelect: (v: string, e: React.MouseEvent) => void;
}) {
  return (
    <div className="relative">
      <button onClick={onToggle} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-slate-200 bg-white text-xs font-medium text-slate-600 hover:bg-slate-50 transition-colors">
        <FilterIcon />
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 w-36 bg-white border border-slate-200 rounded-lg shadow-lg z-20 py-1 animate-fadeIn">
          {options.map((o) => (
            <button key={o} onClick={(e) => onSelect(o, e)}
...
// ─── UploadButton ───
function UploadButton({ onResult }: { onResult: (result: OcrResponse, fileName: string) => void }) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setError(null);
    try {
      const result = await uploadAndExtractOcr(file);
      if (result.success) {
  ...
  return (
    <div className="relative" ref={ref} onClick={(e) => e.stopPropagation()}>
      <input ref={fileInputRef} type="file" accept=".pdf,.jpg,.jpeg,.png,.webp,.bmp,.gif,.xls,.xlsx,.csv" className="hidden" onChange={handleFileChange} />
      <Button
        text={uploading ? "⏳ Extracting..." : "↑ Upload"}
        onClick={() => { setError(null); setOpen((o) => !o); }}
        disabled={uploading}
        className="!px-3 !py-1.5 !text-xs !rounded-lg shadow-sm hover:shadow-md active:scale-95"
      />
      {open && (
        <div className="absolute right-0 top-full mt-1.5 w-36 bg-white border border-slate-200 rounded-xl shadow-xl z-30 py-1.5 animate-fadeIn">
      ...
      {error && (
        <div className="absolute right-0 top-full mt-1.5 w-52 bg-rose-50 border border-rose-200 rounded-lg px-3 py-2 z-30 text-xs text-rose-600 shadow-lg">
          {error}
        </div>
      )}
    </div>
...
function MobileControlCard({ c, index }: { c: Control; index: number }) {
  ...
  return (
    <div className={`px-4 py-3.5 border-b border-slate-50 last:border-0 transition-all duration-300 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-2"}`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        <span className="inline-flex px-2 py-0.5 rounded border border-slate-200 text-xs font-medium text-slate-600 bg-white flex-shrink-0">{c.framework}</span>
...
function TableRow({ c, index }: { c: Control; index: number }) {
  ...
  useEffect(() => { const t = setTimeout(() => setVisible(true), index * 80); return () => clearTimeout(t); }, [index]);
  return (
    <tr className={`border-b border-slate-50 hover:bg-violet-50/30 transition-all duration-300 ${visible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-2"}`}>
      <td className="px-4 py-3 xl:px-5">
        <span className="inline-flex px-2 py-0.5 rounded border border-slate-200 text-xs font-medium text-slate-600 bg-white whitespace-nowrap">{c.framework}</span>
      </td>
...
export default function ComplianceDashboard() {
  ...
  const filtered = activeTab === "All frameworks" ? allControls : allControls.filter((c) => c.framework === activeTab);
  ...
  const handleOcrResult = (result: OcrResponse, fileName: string) => {
    setOcrResult(result);
  ...
  return (
    <>
      <style>{`@keyframes fadeIn { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } } .animate-fadeIn { animation: fadeIn 0.15s ease-out forwards; }`}</style>

      {ocrResult && <OcrResultsModal result={ocrResult} fileName={ocrFileName} onClose={() => setOcrResult(null)} />}
      ...
      <div className="min-h-screen bg-slate-50 font-sans" onClick={closeAll}>
        <main className="w-full px-3 sm:px-5 lg:px-6 py-4 sm:py-5 space-y-3 sm:space-y-4 max-w-screen-2xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2 sm:gap-3">
            <StatCard label="Compliance Rate" rawValue={overallPct} progress={overallPct} cardBg="bg-emerald-50/80" iconBg="bg-emerald-100" icon={<TrendUpIcon />} delay={0} />
            <StatCard label="Compliant" rawValue={compliantCount} cardBg="bg-emerald-50/40" iconBg="bg-emerald-100" icon={<CheckCircleIcon />} delay={80} />
            <StatCard label="In Progress" rawValue={inProgressCount} cardBg="bg-sky-50/50" iconBg="bg-sky-100" icon={<ClockIcon />} delay={160} />
            <StatCard label="Non-Compliant" rawValue={nonCompliantCount} cardBg="bg-rose-50/50" iconBg="bg-rose-100" icon={<XCircleIcon />} delay={240} />
          </div>

          {/* ── 2. Framework Cards ── */}
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
            {frameworkCards.map((fw, i) => <FrameworkCard key={fw} fw={fw} delay={i * 100} />)}
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                {frameworkTabs.map((tab) => (
                  <button key={tab} onClick={() => handleTabChange(tab)}
                    className={`px-2.5 sm:px-3 py-1 rounded-md text-xs font-medium whitespace-nowrap flex-shrink-0 transition-all duration-200 ${
                      activeTab === tab ? "bg-violet-100 text-violet-700 scale-105" : "text-slate-500 hover:bg-slate-100 hover:text-slate-700"}`}>
                    {tab}
                  </button>
                ))}
              </div>
              <div className="flex items-center gap-2 flex-shrink-0">
                <Dropdown value={sortValue} options={["Sort", "A → Z", "Z → A", "Newest"]} open={sortOpen}
                ...
                <Dropdown value={optionValue} options={["Option", "Compliant", "In Progress", "Non-Compliant"]} open={optionOpen}
                ...
                <UploadButton onResult={handleOcrResult} />
              </div>
            </div>
                ...
                <thead>
                  <tr className="border-b border-slate-100">
                    {["Framework", "Control", "Status", "Owner", "Last Review", "Next Review"].map((h) => (
                      <th key={h} className="px-4 xl:px-5 py-2.5 text-left text-xs font-medium text-slate-400 whitespace-nowrap bg-white">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody key={`${activeTab}-${pageKey}`}>
                  {paginated.length === 0 ? (
                    <tr><td colSpan={6} className="px-5 py-10 text-center text-slate-400 text-sm">No controls found.</td></tr>
            ...
            <Pagination currentPage={currentPage} totalItems={filtered.length} itemsPerPage={PAGE_SIZE} onPageChange={handlePageChange} />
          </div>
        </main>
      </div>
    </>
