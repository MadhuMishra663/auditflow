...
export default function UserManagementPage() {
  ...
  const [modal, setModal] = useState<
    { type: "add" } | { type: "edit"; user: User } | null
  >(null);
            ...
            <tbody className="divide-y divide-[#CCC2FF]">
              {paginatedUsers.map((user) => (
                ...
                >
                  ...
                  <td className="px-5 py-4 text-left text-black">
                    {user.email}
                  </td>
                  <td className="px-5 py-4 text-left">
                    <span
                      className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${ROLE_STYLES[user.role]}`}
                    >
                      {user.role}
                    </span>
                  </td>
        ...
        <div className="mt-5 flex items-start gap-3 bg-[#ECE8FF] rounded-xl px-5 py-4 border border-[#CCC2FF]">
          ...
          <div>
            <p className="text-sm font-semibold text-black">
              Demo: Test Different Roles
            </p>
            <p className="text-xs text-black mt-0.5">
              Click the icon to switch between users and see how their role
              affects navigation access and permissions. Currently logged in as:{" "}
              <strong>
